...
docker run -i --rm -u `id -u`:`id -g` -h ${HOSTNAME}-docker --net=host -v /etc/passwd:/etc/passwd:ro -v $(pwd):/builddir ${docker_img_name} /builddir/scripts/builddeb_with_low_tolerance.sh $@
